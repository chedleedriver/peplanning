...
$keywords_list = mysql_query($query_limit_keywords_list, $tp) or die(mysql_error());
$row_keywords_list = mysql_fetch_assoc($keywords_list);

if (isset($_GET['totalRows_keywords_list'])) {
  $totalRows_keywords_list = $_GET['totalRows_keywords_list'];
} else {
  $all_keywords_list = mysql_query($query_keywords_list);
  $totalRows_keywords_list = mysql_num_rows($all_keywords_list);
}
  ...
  <?php do { ?>
    <tr>
      ...
      <td><?php echo $row_keywords_list['keyword']; ?>&nbsp; </td>
    	<td><label onclick="DeleteTableItem('keywords',<? echo $row_keywords_list['id'];?>,'<? echo $_SERVER["PHP_SELF"]?>')">delete</label></td>
	</tr>
    <?php } while ($row_keywords_list = mysql_fetch_assoc($keywords_list)); ?>
</table>
<br>
...
<?php
mysql_free_result($keywords_list);
}
?>
