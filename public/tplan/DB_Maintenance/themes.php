...
$theme = mysql_query($query_limit_theme, $tp) or die(mysql_error());
$row_theme = mysql_fetch_assoc($theme);

if (isset($_GET['totalRows_theme'])) {
  $totalRows_theme = $_GET['totalRows_theme'];
} else {
  $all_theme = mysql_query($query_theme);
  $totalRows_theme = mysql_num_rows($all_theme);
}
...
<table border="1" align="center">
  ...
  <?php do { ?>
    <tr>
	  ...
	  <td><label onclick="DeleteTableItem('theme',<? echo $row_theme['id'];?>,'<? echo $_SERVER["PHP_SELF"]?>')">delete</label></td>
    </tr>
