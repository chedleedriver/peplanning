...
$points = mysql_query($query_limit_points, $tp) or die(mysql_error());
$row_points = mysql_fetch_assoc($points);

if (isset($_GET['totalRows_points'])) {
  $totalRows_points = $_GET['totalRows_points'];
} else {
  $all_points = mysql_query($query_points);
  $totalRows_points = mysql_num_rows($all_points);
}
...
<table border="1" align="center">
  ...
  <?php do { ?>
    <tr>
      ...
      <td><label onclick="DeleteTableItem('point',<? echo $row_points['id'];?>,'<? echo $_SERVER["PHP_SELF"]?>')">delete</label></td>
	</tr>
