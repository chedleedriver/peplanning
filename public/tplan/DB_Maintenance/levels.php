...
$levels = mysql_query($query_limit_levels, $tp) or die(mysql_error());
$row_levels = mysql_fetch_assoc($levels);

if (isset($_GET['totalRows_levels'])) {
  $totalRows_levels = $_GET['totalRows_levels'];
} else {
  $all_levels = mysql_query($query_levels);
  $totalRows_levels = mysql_num_rows($all_levels);
}
...
<table border="1" align="center">
  ...
  <?php do { ?>
    <tr>
	  ...
	  <td><label onclick="DeleteTableItem('level',<? echo $row_levels['id'];?>,'<? echo $_SERVER["PHP_SELF"]?>')">delete</label></td>
    </tr>
    <?php } while ($row_levels = mysql_fetch_assoc($levels)); ?>
</table>
<br>
