 ...
 * http://localhost/tplan/DB_Maintenance/move_unit.php?user_id=1&from=peplanning-dev&to=localhost&database_from=tplan&database_to=tplan
 ...
 *
 */
$tp = mysql_pconnect("peplanning-dev", "tp", "enquiry") or die(mysql_error()." - PEplanning-dev");
mysql_select_db("tplan", $tp) or die(mysql_error()." - tplan");
$user_id=1;
        ...
        while(list($unit_id,$description,$topic_id,$level_id,$num_lessons,$topic)=mysql_fetch_array($got_units))
                {
                    echo "</br>Getting Unit - ".$unit_id." - ".$description."</br>";
                        for ($i=1;$i<=$num_lessons;$i++)
                            ...
                            mysql_select_db("tplan", $tp) or die(mysql_error());;
                            //get the unit id
                            $unit_id_sql="select id from unit_of_work where teacher_id = 1 and topic_id = $topic_id";
                            $unit_id_result=mysql_query($unit_id_sql)  or die("Error reported while executing the statement: $unit_id_sql<br />MySQL reported: ".mysql_error());
                            list($unit_id)=mysql_fetch_array($unit_id_result);
                            echo "</br>Saving Unit ".$unit_id." - ".$description."</br>";
                            //delete the lessons
                            $delete_sql = "delete from lesson where uow_id = $unit_id";
