...
$topics_list = mysql_query($query_limit_topics_list, $tp) or die(mysql_error());
$row_topics_list = mysql_fetch_assoc($topics_list);

if (isset($_GET['totalRows_topics_list'])) {
  $totalRows_topics_list = $_GET['totalRows_topics_list'];
} else {
  $all_topics_list = mysql_query($query_topics_list);
  $totalRows_topics_list = mysql_num_rows($all_topics_list);
}
...
<table border="1" align="center">
  ...
  <?php do { ?>
    <tr>
	  ...
	  <td><label onclick="DeleteTableItem('topic',<? echo $row_topics_list['id'];?>,'<? echo $_SERVER["PHP_SELF"]?>')">delete</label></td>
    </tr>
    <?php } while ($row_topics_list = mysql_fetch_assoc($topics_list)); ?>
</table>
<br>
