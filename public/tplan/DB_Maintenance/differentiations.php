...
$differentiations = mysql_query($query_limit_differentiations, $tp) or die(mysql_error());
$row_differentiations = mysql_fetch_assoc($differentiations);

if (isset($_GET['totalRows_differentiations'])) {
  $totalRows_differentiations = $_GET['totalRows_differentiations'];
} else {
  $all_differentiations = mysql_query($query_differentiations);
  $totalRows_differentiations = mysql_num_rows($all_differentiations);
}
...
<table border="1" align="center">
  ...
  <?php do { ?>
    <tr>
	  ...
	  <td><label onclick="DeleteTableItem('differentiation',<? echo $row_differentiations['id'];?>,'<? echo $_SERVER["PHP_SELF"]?>')">delete</label></td>
    </tr>
    <?php } while ($row_differentiations = mysql_fetch_assoc($differentiations)); ?>
</table>
<br>
...
<?php }
mysql_free_result($differentiations);
?>
