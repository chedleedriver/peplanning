...
$lesson_parts = mysql_query($query_limit_lesson_parts, $tp) or die(mysql_error());
$row_lesson_parts = mysql_fetch_assoc($lesson_parts);

if (isset($_GET['totalRows_lesson_parts'])) {
  $totalRows_lesson_parts = $_GET['totalRows_lesson_parts'];
} else {
  $all_lesson_parts = mysql_query($query_lesson_parts);
  $totalRows_lesson_parts = mysql_num_rows($all_lesson_parts);
}
...
<table border="1" align="center">
  ...
  <?php do { ?>
    <tr>
      ...
      <td><label onclick="DeleteTableItem('lesson_part',<? echo $row_lesson_parts['id'];?>,'<? echo $_SERVER["PHP_SELF"]?>')">delete</label></td></tr>
    <?php } while ($row_lesson_parts = mysql_fetch_assoc($lesson_parts)); ?>
</table>
<br>
...
<?php
mysql_free_result($lesson_parts);
}
?>
