       ...
       objectName.onreadystatechange=function(){    
         if (objectName.readyState==4) {
            ...
            if (planType=='newPlan'&&dataArray[2]=='R'&&dataArray[0]!=9){
                alert('You may only choose \'Select Preset PE Lessons\' with this activity');
            }
            else {
...
function trialNotification(){
    alert('As a free trial user your access is restricted to Lesson 1 only.\n\nPlease click \'Upgrade Now\' to subscribe in full');
}
function createDefaultUnit(saveTopicId,saveTopic,saveNumLessons,saveLevel,saveDescription){
    $('form')[0].reset();
    $('#accordion2').html("<div class='loading'>loading plan...</div>");
    $('#accordion3').html("<div class='loading'>loading assessments...</div>");
    document.location=("main.php#feature-plans");
    ...
    urlstring = 'code/basefunctions.php?CreateDefaultUnit=1&topic_id='+saveTopicId+'&topic='+saveTopic+'&num_lessons='+saveNumLessons+'&level='+saveLevel+'&description='+saveDescription;
    objectName.open("GET", urlstring, true);
    objectName.send(null);
}
...
function freePrintPlan(planNum,unitId) {
   if (planNum==0)
        {
            var $tabs=$('#tabs').tabs();
            planNum=$tabs.tabs('option', 'selected')+1;
    ...
    //window.goLocation=('code/plan_print.php?lesson_num=' + planNum + '&unit_id=' + unitId);
    urlstring = 'code/free_plan_print.php?lesson_num=' + planNum + '&unit_id=' + unitId;
    objectName.open("GET", urlstring, false);
    objectName.send(null);
}
...
$.uicornerfix = function(r){
  ...
  DD_roundies.addRule('.btn-m', r+' '+r+' '+r+' '+r);
  DD_roundies.addRule('.help',  r+' '+r+' '+r+' '+r);
  DD_roundies.addRule('#signin_menu',  r+' '+r+' '+r+' '+r);
};
