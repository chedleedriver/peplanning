/*
 * jquery.layout 1.3.rc5 - Release Candidate 5
 *
 * Copyright (c) 2009 
 ...
 *
 * $Date: 2009-09-19 08:00:00 -0800 (Tue, 19 Sep 2009) $
 * $Rev: 203 $
 * 
 ...
 */
	...
	,	resizeWithWindowDelay:		150			// delay calling resizeAll because makes window resizing very jerky
		...
		try {
			...
			if (typeof fn=="function") {
				if (pane && $Ps[pane])
					...
					return fn( 'container', $Container, $.extend({},state.container), options, options.name );
			}
		}
		catch (ex) {}
	};
 ...
 */
	...
	var create = function () {
		...
		if (o.autoStateManagement)
			$(window).bind("unload."+ sID, function(){
	...
	var initContainer = function () {
		...
		$Container.data("layoutContainer", sID);

		// SAVE original container CSS for use in destroy()
		if (!$Container.data("layoutCSS")) {
			var
		...
		try { // format html/body if this is a full page layout
			if (isFullPage) {
				...
				if ($Container.is(":visible") && $Container.innerHeight() < 2)
					alert( lang.errContainerHeight.replace(/CONTAINER/, $Container[0].tagName + ($Container.selector || '')) );
			}
		} catch (ex) {}
	...
	var bindStopSlidingEvents = function (pane, enable) {
		...
		if (enable) { // BIND trigger events
			...
			if (trigger == "mouseout") {
				...
				$R.bind("dblclick."+ sID, close ); // ALWAYS close with dbl-click on Slider-bar
...
if (pane == "center") {
	alert(
		pane +'\n=============\n'+
		'state.noRoom = '+ s.noRoom +'\n'+
		'state.minSize = '+ s.minSize +'\n'+
		'state.maxSize = '+ s.maxSize +'\n'+
		'state.minWidth = '+ s.minWidth +'\n'+
	...
	debugData(s, "center");
		...
		else if (!s.noRoom) { // pane CANNOT fit, but state is currently INCORRECT, so set it...
	...
	var sizePane = function (pane, size, skipCallback) {
		...
		if (!s.isSliding) sizeMidPanes(_c[pane].dir=="horz" ? "all" : "center");
		sizeHandles("all");
		sizeContent(pane);
	...
	*/
	...
	function getCookie () {
		...
		for (var i=0, n=cs.length; i < n; i++) {
			...
			if (pair[0] == name) { // found the layout cookie
				// convert cookie string back to a hash - handle invalid data
				try { return window["eval"]("("+ decodeURIComponent(pair[1]) +")") || {}; }
				catch (e) {}
				break; // DONE
			}
		}
	...
	function saveState (keys, opts) {
		...
		if (clear) {
			...
			document.cookie = name +'='+ encodeURIComponent( stringify() ) + params; // write cookie
		}

		return $.extend( {}, state.cookie ); // return COPY of state.cookie

		/*
		 * Stringify the state.cookie hash so can save in the cookie
		 */
		function stringify () {
			return parse( state.cookie );
			function parse (data) {
				var D=[], i=0, k, v, t; // k = key, v = value
				for (k in data) {
					v = data[k];
					if (typeof v == 'string')		// STRING - add quotes
						v = '"'+ v +'"';
					else if (typeof v == 'object')	// SUB-KEY - recurse into it
						v = parse(v);
					D[i++] = '"'+ k +'":'+ v;
				}
				return "{"+ D.join(",") +"}";
