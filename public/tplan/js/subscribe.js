function isBlank(A) {
    ...
    function A() {
        ...
        F.find("#school_name").isSignupFormField({
            validateWith: function (H) {
                return isBlank(H) ? _("Should be a name") : true
            }
        });
        F.find("#user_postcode").isSignupFormField({
            validateWith: function (I) {
                return isBlank(I) ? _("Should be a valid postcode") : true
            }
        });
        F.find("#address-1").isSignupFormField({
            validateWith: function (J) {
                return isBlank(J) ? _("Should be an address line") : true
            }
        });
        F.find("#address-2").isSignupFormField({
            validateWith: function (K) {
                return isBlank(K) ? _("Should be an address line") : true
            }
        });
        F.find("#address-3").isSignupFormField({
            validateWith: function (L) {
                return isBlank(L) ? _("Should be an address line") : true
            }
        });
        F.find("#telephone").isSignupFormField({
            validateWith: function (M) {
                return isBlank(M) ? _("Should be an address line") : true
            }
        });
        F.find("#class-num").isSignupFormField({
            validateWith: function (N) {
                return isBlank(N) ? _("Should be a number") : true
            }
        });
        F.find("#sub-from").isSignupFormField({
            validateWith: function (O) {
                return isBlank(O) ? _("Should be a date") : true
            }
        });
        F.find("#sub-to").isSignupFormField({
            validateWith: function (P) {
                return isBlank(P) ? _("Should be a date") : true
            }
        });
        F.find("#school_name").isScreenNameField();
        F.find("#user_email").isEmailField();
        F.find("#password").isSignupFormField({
            allowInput: /[^\s]/
        }).focus(function () {
        ...
        return this.each(function () {
            ...
            function B() {
                ...
                if (I.match(C)) {
                    jQuery.ajax({
                        type: "GET",
                        url: "/tplan/code/basefunctions.php",
                        data: {
                            school_email: I
                        },
                        dataType: "json",
...
(function (A) {
    A.fn.isScreenNameField = function () {
        return this.each(function () {
            var M = A(this);
            var F = A("#signup_schoolname_url");
            var E = A("#schoolname_info");
            var D = A("#avail_schoolname_check_indicator");
            var O;
            var C;
            var I;
            var N = M.val().stripTags();
            M.val(N);
            var G = N;
            var H = N != "";
            var Q = /[a-zA-Z0-9 _]/;
            ...
            M.isSignupFormField({
                validateWith: function (R) {
                    if (isBlank(R)) {
                        return _("Please enter a school name")
                    } else {
                        P()
                    }
                },
            ...
            M.keyup(function (S) {
                if (jQuery.inArray(S.keyCode, [16, 17, 18, 20, 27, 33, 34, 35, 37, 38, 39, 40, 144]) == -1) {
                    var R = M.val();
                    if (R != "") {
                        M.val(R.stripTags());
                        H = true
        ...
        return this.each(function () {
            ...
            if (N) {
                K.keypress(function (b) {
                    ...
                    return !!(a == 0 || a == 8 || a == 9 || a == 13 || a == 65 || c.match(N))
                })
            }
            F.hide()
        })
