    $(document).ready(function() {
        ...
        var $tabs=$('#tabs').tabs({
			...
			panelTemplate:'<div style="height:650px"></div>',
                        tabTemplate: '<li id="#{label}" class="ui-widget"><a href="#{href}" title="Lesson"><span>#{label}</span></a> <span class="ui-icon ui-icon-close">Remove Tab</span></li>',
			ajaxOptions: {
                            async: false,
                            cache:false,
...
for (i = 1 ; i <= window.numLessons ; i++) {
    ...
    $('#tabs').tabs("url",i-1,lessonTabUrl);
}
