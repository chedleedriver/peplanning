...
$evaluation = mysqli_query($tp, $query_limit_evaluation) or die(mysqli_error());
...
if (isset($_GET['totalRows_evaluation'])) {
  ...
  $all_evaluation = mysqli_query($tp, $query_evaluation);
  $totalRows_evaluation = mysqli_num_rows($all_evaluation);
}
...
?>
    <tr>
	  ...
	  <td><label onclick="DeleteTableItem('evaluation',<?php echo $row_evaluation['id'];?>,'<?php echo $_SERVER["PHP_SELF"]?>')">delete</label></td>    
	</tr>
    <?php } while ($row_evaluation = mysqli_fetch_assoc($evaluation)); ?>
</table>
<br>
