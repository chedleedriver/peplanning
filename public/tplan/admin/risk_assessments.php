...
$risk_assessment = mysqli_query($tp, $query_limit_risk_assessment) or die(mysqli_error());
$row_risk_assessment = mysqli_fetch_assoc($risk_assessment);

if (isset($_GET['totalRows_risk_assessment'])) {
  $totalRows_risk_assessment = $_GET['totalRows_risk_assessment'];
} else {
  $all_risk_assessment = mysqli_query($tp, $query_risk_assessment);
  $totalRows_risk_assessment = mysqli_num_rows($all_risk_assessment);
}
...
<table border="1" align="center">
  ...
  <?php do { ?>
    <tr>
	  ...
	  <td><label onclick="DeleteTableItem('risk_assessment',<?php echo $row_risk_assessment['id'];?>,'<?php echo $_SERVER["PHP_SELF"]?>')">delete</label></td>
    </tr>
    <?php } while ($row_risk_assessment = mysqli_fetch_assoc($risk_assessment)); ?>
</table>
<br>
...
<?php
mysqli_free_result($risk_assessment);
}
?>
