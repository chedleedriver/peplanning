...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE evaluation SET `evaluation`=%s WHERE id=%s",
  ...
  echo("<meta http-equiv=\"Refresh\" content=\"0;url=/tplan/DB_Maintenance/evaluations.php\">");
}
$evaluation_id=$_GET['recordID'];

$query_evaluation_update = "SELECT * FROM evaluation where id = $evaluation_id";
$evaluation_update = mysqli_query($tp, $query_evaluation_update) or die(mysqli_error());
$row_evaluation_update = mysqli_fetch_assoc($evaluation_update);
$totalRows_evaluation_update = mysqli_num_rows($evaluation_update);
...
<form method="post" name="form1" action="<?php echo $editFormAction; ?>">
  <table align="center">
    <tr valign="baseline">
      <td nowrap align="right">evaluation_id:</td>
      <td><?php echo $row_evaluation_update['id']; ?></td>
    </tr>
    
    <tr valign="baseline">
      <td nowrap align="right">Description:</td>
      <td><table>
	  <?php $description=unserialize($row_evaluation_update['evaluation']);?>
  ...
  <input type="hidden" name="id" value="<?php echo $row_evaluation_update['id']; ?>">
</form>
<p>&nbsp;</p>
</body>
</html>
<?php }
mysqli_free_result($evaluation_update);
?>
