...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE objectives SET assessment=%s  WHERE id=%s",
                       GetSQLValueString($_POST['assessment'], "text"),
                       GetSQLValueString($_POST['id'], "int"));
  echo $updateSQL;
  
  $Result1 = mysqli_query($tp, $tp, $updateSQL) or die(mysqli_error());
  if ($_POST['themes']) Update_theme_objectives($_POST['themes'],$_POST['id']);
  
  $updateGoTo = "choose_objective.php?mode=objective&topic=".$_GET['topic'];
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING'];
  }
  echo("<meta http-equiv=\"Refresh\" content=\"0;url=/tplan/DB_Maintenance/choose_objective.php?mode=objective&topic=".$_GET['topic']."\">");
...
<form method="post" name="form1" action="<?php echo $editFormAction; ?>">
  <table align="center">
    ...
    <tr valign="baseline">
      <td nowrap align="right">Assessment:</td>
      <td><input type="text" name="assessment" value="<?php echo $row_objective_update['assessment']; ?>" size="32" maxlength="128"></td>
    </tr>
    <tr valign="baseline">
