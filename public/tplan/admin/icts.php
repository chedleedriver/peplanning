...
$ict = mysqli_query($tp, $query_limit_ict) or die(mysqli_error());
$row_ict = mysqli_fetch_assoc($ict);

if (isset($_GET['totalRows_ict'])) {
  $totalRows_ict = $_GET['totalRows_ict'];
} else {
  $all_ict = mysqli_query($tp, $query_ict);
  $totalRows_ict = mysqli_num_rows($all_ict);
}
...
<table border="1" align="center">
  ...
  <?php do { ?>
    <tr>
	  ...
	  <td><label onclick="DeleteTableItem('ict',<?php echo $row_ict['id'];?>,'<?php echo $_SERVER["PHP_SELF"]?>')">delete</label></td>
    </tr>
    <?php } while ($row_ict = mysqli_fetch_assoc($ict)); ?>
</table>
<br>
