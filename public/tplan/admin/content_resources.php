...
{
    list($cr_id,$c_id,$description,$name,$location,$type)=mysqli_fetch_array(mysqli_query($tp, "select * from content_resources where id=$cr_id")) or die ("<br />MySQL reported: ".mysqli_error());
}
if ($mode=="look")
	...
	$resource_results = mysqli_query($tp, $resource_sql) or die("Error reported while executing the statement: $resource_sql<br />MySQL reported: ".mysqli_error());
}
...
<form method="post" name="form1" action="content_resources.php?mode=update" enctype="multipart/form-data">
    <input type="hidden" name="content_id" value="<?php echo $content_id?>" size="4" /><input type="hidden" name="topic" id="topic" value="<?php echo $topic_id?>" /><input type="hidden" name="cr_id" value="<?php echo $cr_id?>" size="4" />
  <table align="center">
<?php if ($mode=="look")
{?>
    <tr valign="baseline">
      <td nowrap="true" align="right">Name:</td>
      <td><input type="text" name="content_name" value="<?php echo $content_name?>" size="32" maxlength="1024"/></td>
    </tr>
      <tr>
...
<td><input type="button" id="return" onclick="ReturnTo('index',0)" value="return to menu" /></td>
    ...
    <table border="2" align="center">
        ...
        <?php while (list($cr_id,$c_id,$description,$name,$location,$type)=mysqli_fetch_array($resource_results))
        {?>
        <tr>
