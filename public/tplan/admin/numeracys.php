...
$numeracy = mysqli_query($tp, $query_limit_numeracy) or die(mysqli_error());
$row_numeracy = mysqli_fetch_assoc($numeracy);

if (isset($_GET['totalRows_numeracy'])) {
  $totalRows_numeracy = $_GET['totalRows_numeracy'];
} else {
  $all_numeracy = mysqli_query($tp, $query_numeracy);
  $totalRows_numeracy = mysqli_num_rows($all_numeracy);
}
...
<table border="1" align="center">
  ...
  <?php do { ?>
    <tr>
	  ...
	  <td><label onclick="DeleteTableItem('numeracy',<?php echo $row_numeracy['id'];?>,'<?php echo $_SERVER["PHP_SELF"]?>')">delete</label></td>
    </tr>
    <?php } while ($row_numeracy = mysqli_fetch_assoc($numeracy)); ?>
</table>
<br>
