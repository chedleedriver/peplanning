...
$genre_list = mysqli_query($tp, $query_limit_genre_list) or die(mysqli_error());
$row_genre_list = mysqli_fetch_assoc($genre_list);

if (isset($_GET['totalRows_genre_list'])) {
  $totalRows_genre_list = $_GET['totalRows_genre_list'];
} else {
  $all_genre_list = mysqli_query($tp, $query_genre_list);
  $totalRows_genre_list = mysqli_num_rows($all_genre_list);
}
  ...
  <?php do { ?>
    <tr>
      ...
      <td><?php echo $row_genre_list['description']; ?>&nbsp; </td>
    	<td><label onclick="DeleteTableItem('genre',<?php echo $row_genre_list['id'];?>,'<?php echo $_SERVER["PHP_SELF"]?>')">delete</label></td>
    </tr>
    <?php } while ($row_genre_list = mysqli_fetch_assoc($genre_list)); ?>
</table>
<br>
