...
where id=$id)";
		...
		elseif ($what=="objectives") $sql = "select * from ".$what." where topic_id=$id order by objective";
...
<? } elseif ($what_to_add=="evaluation")  {$description=unserialize($name);$name='';foreach ($description as $description_line)
...
<option onclick="GetLevel(<? echo $id.",".$topic_id?>)" value=<? echo $id?>><? echo $name?></option>
<? } elseif ($what_to_remove=="evaluation")  {$description=unserialize($name);$name='';foreach ($description as $description_line)
{if ($description_line) $name=$name." ".$description_line;}
