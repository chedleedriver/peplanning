...
function GetNumLessons($unit_id)
{
    ...
    $result=mysql_query($sql) or die("Error reported while excetung the statement; $sql<br />MySQL reportedL  ".mysql_error());
    list($num_lessons)=  mysql_fetch_array($result);
    return($num_lessons);
}
...
$sql="select name,description,time from content where content_id=$content_id";
	...
	if ($strand)
		...
		$image_file="/var/www/html/peplanning/public/tplan/diagrams/".$content_id.".gif";
...
function DrawPlanPage($lesson_num,$theme_id,$topic_id,$level_id,$uow_id,$lesson_objectives,$lesson_id,$plan_type)
{
	...
	{ 
		...
		echo "<script type=\"text/javascript\">;window.lessonType='newLesson';";
            	SetUpNewLesson($uow_id,$lesson_num,$theme_id);
		echo "</script>";
 ...
 function GetPrintContent($lesson_num,$uow_id)
{
   ...
   //get the topic details
	...
	list ($_SESSION[$lesson_num]['mysubject'])=mysql_fetch_array($topic_name);
   // get the school, this may need some thought regarding setting their accounts up
	...
	list($_SESSION[$lesson_num]['school'])=mysql_fetch_array($school_name);
   ...
   // get the assistance details
        ...
        $_SESSION[$lesson_num]['assistance']=mysql_query("select ta,sen from lesson where id=$lesson_id");
...
//        $evaluations=mysql_query("select evaluation.evaluation from lesson left join theme on lesson.theme_id=theme.id left join topic_theme on theme.id=topic_theme.theme_id left join topic_theme_evaluation on topic_theme_evaluation.theme_id=topic_theme.theme_id left join evaluation on evaluation.id=topic_theme_evaluation.evaluation_id where topic_theme.topic_id=$topic_id and topic_theme_evaluation.level=$level_id and evaluation.evaluation is not NULL order by evaluation.evaluation desc");
  ...
  // get the objectives for the lesson and store in an array
        $_SESSION[$lesson_num]['objectives']=mysql_query("select objective_id,objective from lesson_objectives left join lesson on lesson.id=lesson_objectives.lesson_id left join objectives on lesson_objectives.objective_id=objectives.id where lesson.id=$lesson_id order by objectives.id");
	$_SESSION[$lesson_num]['keywords']=mysql_query("select keyword_id,keyword from lesson_keywords left join keywords on keywords.id=lesson_keywords.keyword_id left join lesson on lesson.id=lesson_keywords.lesson_id where lesson.id=$lesson_id");
	$_SESSION[$lesson_num]['icts']=mysql_query("select ict_id,description from lesson_ict left join ICT on ICT.id=ict_id left join lesson on lesson.id=lesson_ict.lesson_id where lesson.id=$lesson_id");
	$_SESSION[$lesson_num]['numeracys']=mysql_query("select numeracy_id,description from lesson_numeracy left join numeracy on numeracy.id=numeracy_id left join lesson on lesson.id=lesson_numeracy.lesson_id where lesson.id=$lesson_id");
	$_SESSION[$lesson_num]['citizenships']=mysql_query("select citizenship_id,description from lesson_citizenship left join citizenship on citizenship.id=citizenship_id left join lesson on lesson.id=lesson_citizenship.lesson_id where lesson.id=$lesson_id");
	$_SESSION[$lesson_num]['risk_assessments']=mysql_query("select ra_id,description from lesson_risk_assessment left join risk_assessment on risk_assessment.id=ra_id left join lesson on lesson.id=lesson_risk_assessment.lesson_id where lesson.id=$lesson_id");
	$_SESSION[$lesson_num]['equipment']=mysql_query("select la_id,activity_id,lesson_activities.time as la_time,lesson_part.description as lesson_part,content.name as content_name,content.description as content_description,content.time as content_time, equipment.description as equipment from lesson_activities left join content on content.content_id=activity_id left join lesson on lesson.id=lesson_activities.lesson_id left join content_lesson_part on content_lesson_part.content_id = lesson_activities.activity_id left join lesson_part on lesson_part.id = content_lesson_part.lesson_part_id left join content_equipment on activity_id=content_equipment.content_id left join equipment on content_equipment.equipment_id=equipment.id where lesson.id=$lesson_id order by activity_num");
	$_SESSION[$lesson_num]['activities']=mysql_query("select la_id,activity_id,lesson_activities.time as la_time,lesson_part.description as lesson_part,content.name as content_name,content.description as content_description,content.time as content_time from lesson_activities left join content on content.content_id=activity_id left join lesson on lesson.id=lesson_activities.lesson_id left join content_lesson_part on content_lesson_part.content_id = lesson_activities.activity_id left join lesson_part on lesson_part.id = content_lesson_part.lesson_part_id where lesson.id=$lesson_id order by activity_num");
	if ($_SESSION[$lesson_num]['activities'])
	{
		$act_num=1;
		$num_acts=mysql_num_rows($_SESSION[$lesson_num]['activities']);
		while (list($la_id,$activity_id,$time,$lesson_part,$content_name,$content_description,$content_time)=mysql_fetch_array($_SESSION[$lesson_num]['activities']))
		{
			if ($lesson_part) $lesson_part_string[$act_num]=$lesson_part;
                                $strands=mysql_query("select strand_id,strand from lesson_activity_strand left join strand on strand.id=strand_id left join lesson_activities on lesson_activities.la_id=lesson_activity_strand.la_id where lesson_activity_strand.la_id = $la_id");
				if ($strands)
				...
				$_SESSION[$lesson_num]['progressions'][$act_num]=mysql_query("select la_pr_id,pr_id,progression from lesson_activity_progression left join progression on progression.id=pr_id left join lesson_activities on lesson_activities.la_id=lesson_activity_progression.la_id where lesson_activity_progression.la_id = $la_id");
				if ($_SESSION[$lesson_num]['progressions'][$act_num])
				{
				       $_SESSION[$lesson_num]['num_progs'][$act_num]=mysql_num_rows($_SESSION[$lesson_num]['progressions'][$act_num]);
                                        for ($i = 0 ; $i <= count($_SESSION[$lesson_num]['num_progs'][$act_num]) ; $i++)
                                        {
                                            list($la_pr_id,$progression_id,$progression)=mysql_fetch_array($_SESSION[$lesson_num]['progressions'][$act_num]);
					    $_SESSION[$lesson_num]['prog_points'][$act_num][$i]=mysql_query("select point_id,point from lesson_activity_progression_point left join lesson_activity_progression on lesson_activity_progression.la_pr_id=lesson_activity_progression_point.la_pr_id left join point on point.id=point_id where  lesson_activity_progression.la_id = $la_id and lesson_activity_progression.pr_id=$progression_id order by point.id desc");
                                            //$_SESSION[$lesson_num]['prog_points'][$act_num][$i]=mysql_query("select point_id,point from lesson_activity_progression_point left join lesson_activity_progression on lesson_activity_progression.la_pr_id=lesson_activity_progression_point.la_pr_id left join point on point.id=point_id where  lesson_activity_progression_point.la_pr_id = $la_pr_id");
                                        }
				}
				$_SESSION[$lesson_num]['analysess'][$act_num]=mysql_query("select analyses_id,content.name as analysis_title,content.description as analysis from lesson_activity_analyses left join content on content.content_id=analyses_id left join lesson_activities on lesson_activities.la_id=lesson_activity_analyses.la_id where lesson_activity_analyses.la_id = $la_id");
				if ($_SESSION[$lesson_num]['analysess'][$act_num])
				{
				      	$num_anals=mysql_num_rows($_SESSION[$lesson_num]['analysess'][$act_num]);
                                        for ($i = 0 ; $i < $num_anals ; $i++)
                                            {
						list($analyses_id,$content_name,$content_description)=mysql_fetch_array($_SESSION[$lesson_num]['analysess'][$act_num]);
                                                $_SESSION[$lesson_num]['anal_points'][$act_num][$i]=mysql_query("select point_id,point from point left join content_point on content_point.point_id=point.id where content_point.content_id=$analyses_id");
                                            }
                                }
		$act_num++;
		}
                if (mysql_num_rows($_SESSION[$lesson_num]['activities'])>0) mysql_data_seek($_SESSION[$lesson_num]['activities'],0);
	}
}
...
function SetUpThemes($unit_id,$l)
{
...
function SetUpNewLesson($unit_id,$l,$theme_id)
{
	$lessons=mysql_query("select id from lesson where uow_id=$unit_id and lesson_num=$l") or die("Error reported while executing the statement: <br />MySQL reported: ".mysql_error());
	while(list ($lesson_id)=mysql_fetch_array($lessons))
		{
...
/*			echo "objectiveIds[".$l."].splice(objectiveIds[".$l."].length,0,".$objective_id.");";
	...
	if ($activities)
	{
	echo "window.activityCount[".$l."]=".mysql_num_rows($activities).";";
...
function DeleteLesson($uow_id,$lesson_num,$DeleteLesson)
{
    ...
    if (mysql_num_rows($getlessonnum)>0)
    {
        ...
        $removeoldrisk_assessment=mysql_query("delete from lesson_risk_assessment where lesson_id=$lesson_id");
        $removeoldcitizenship=mysql_query("delete from lesson_citizenship where lesson_id=$lesson_id");
        $getlessonactivities=mysql_query("select la_id from lesson_activities where lesson_id=$lesson_id");
        if (mysql_num_rows($getlessonactivities)>0)
        {
            while (list ($la_id)=mysql_fetch_array($getlessonactivities))
            {
                $deleteteachpoint=mysql_query("delete from lesson_activity_point where la_id=$la_id");
                $deleteteachstrand=mysql_query("delete from lesson_activity_strand where la_id=$la_id");
                $deletediffs=mysql_query("delete from lesson_activity_differentiation where la_id=$la_id");
                $getprogressions=mysql_query("select la_pr_id from lesson_activity_progression where la_id=$la_id");
                if(mysql_num_rows($getprogressions)>0)
                {
                    while (list ($la_pr_id)=mysql_fetch_array($getprogressions))
                    {
                        $deleteprogressionpoints=mysql_query("delete from lesson_activity_progression_point where la_pr_id=$la_pr_id");
                    }
                }
                $deleteprogressions=mysql_query("delete from lesson_activity_progression where la_id=$la_id");
                $getanals=mysql_query("select la_pr_id from lesson_activity_analyses where la_id=$la_id");
                if(mysql_num_rows($getanals)>0)
                {
                    while (list ($la_an_id)=mysql_fetch_array($getanals))
                    {
                        $deleteanalpoints=mysql_query("delete from lesson_activity_analyses_point where la_pr_id=$la_pr_id");
                    }
                }
                $deleteanals=mysql_query("delete from lesson_activity_analyses where la_id=$la_id");
            }
            $removeoldactivity=mysql_query("delete from lesson_activities where lesson_id=$lesson_id");
        }
        $removeoldlesson=mysql_query("delete from lesson where lesson_id=$lesson_id");
    }
}
