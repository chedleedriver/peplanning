...
function passwordFocus(from)
{
    ...
    if(from=='signup') {var clearField='#signup_password_clear'; var passField='#signup_password';}
    if(from=='signup_0') {var clearField='#signup_0_password_clear'; var passField='#signup_0_password';}
    if(from=='signup_1') {var clearField='#signup_1_password_clear'; var passField='#signup_1_password';}
    if(from=='signup_2') {var clearField='#signup_2_password_clear'; var passField='#signup_2_password';}
    if(from=='signup_3') {var clearField='#signup_3_password_clear'; var passField='#signup_3_password';}
    ...
    jSelectform('<form id="assess_form"><h1>You can assess your unit in 3 ways:</h1><select class="popup_select" id="assess_sel"><option value=1>Class Assessment</option><option value=2>Individual Assessment</option><option value=3>Teacher Assessment</option><p><a href="/learn/assessment"><span class=info>Not sure which to choose?</span></a></form>', '', 'Choose Assessment Type', function(r) {
       if((r=='1')||(r=='2')||(r=='3')){
           ...
           $.ajax({
            ...
            location.href="/tplan/code/teacher_assessment_2.php?unit_id=" + varUnit + "&level_id=" + varLevel;
        }
       }
        else{
            if(r!=null){ 
...
function deleteUnit(unitId)
{
    ...
    if (r) {
		showStatus('Delete','Deleting...');
		var deleteTime = 10;
        var stat=setInterval("updateStatus("+deleteTime+")",500);
        $.ajax({
        url:'/tplan/code/lessonfunctions_2.php',
        data:{DeleteUnit:1,unit_id:unitId},
        dataType:'json'})
            .done(function(msg){
                $.alerts._hide();
                var response=eval(msg);
...
function getLessonNumbers(topic_id)
{
    ...
    if((topic==53)||(topic==54)||(topic==55)||(topic==56)||(topic==61)||(topic==62)||(topic==63)) $("#sel_num_lessons").append("<option value=7>7</option>");
    else if((topic==57)||(topic==58)||(topic==59)||(topic==60)||(topic==64)) $("#sel_num_lessons").append("<option value=6>6</option>");
    else {
        for(var i=5;i<=12;i++){
...
function showStatus(whatStatus,whatDoing)
{
    $('#status_bar').progressbar("destroy");
    jStatus('',whatStatus,whatDoing);
}
function updateStatus(time)
{
    var numTime = parseFloat(time);
	var interval = 100 / numTime;
	var status = parseFloat($('#status_bar').progressbar("value"));
	var statusValue = interval + status;
	$('#status_bar').progressbar({value:statusValue});
}
...
function doProcess(proc)
{
	...
	if(proc=='Subscribe') sendData=sendData+'&recaptcha_challenge_field='+$('#recaptcha_challenge_field').val()+'&recaptcha_response_field='+$('#recaptcha_response_field').val();
    $(".error").val('');
...
function doContact()
{
    ...
    $.ajax({
        type:"POST",
        url:"/auth/contact",
        data:{name_contact:name_contact,email_contact:email_contact,details_contact:details_contact,nature_contact:nature_contact,'captcha[input]':captcha['input'],'captcha[id]':captcha['id']},
        dataType:"json"})
        .done(function(msg){
            processContact(msg);
        });
...
function processContact(data)
{
    ...
    if(response.result==1){
        ...
        //alert('response : '+response.result+' detail : '+response.detail);
        for(var key in response){ 
            if(key=='captchahtml')
              {
                //alert( response[key].substr(67,290));
                $("#captcha-element").html(response[key].substr(67,290));
                //trying to extract the bit we need
              }
            else if(key=='detail')
              {
               for (var subKey in response[key]){
                var fieldName = '#' + subKey + '_msg';
                   //for (var errorMsg in response.key.subKey){
                     //alert ('fieldname = '+fieldName); 
                     //alert ('error message = '+response.key.subKey[errorMsg]);  
                     //$(fieldName).val(response.key.subKey[errorMsg]);
                     //}
                 $(fieldName).val('please check this value');
               }      
            }
        }
        //if(response.captchahtml) alert(response.captchahtml);
   }
...
function downloadFile(file,level)
{
    ...
    else if(file=='GymnasticsMarketingLesson.pdf') document.location = '/resource-downloads/' + file;
...
/*-------New Interface for the planning tool-----------------------------*/
function fadeDashBoard(){
      for(var i = 1; i <= 3; i++) {
        $('.col'+i).addClass('fadeOutback').removeClass('fadeInForward-'+i);
      }
	  $('.multicol').addClass('fadeOutback').removeClass('fadeInForward-multi');
    }
function showDashBoard(){
	  ...
	  $('.multicol').addClass('fadeInForward-multi').removeClass('fadeOutback');
    }  
/*-------Make selection and pass to server for cookie storage------------*/
function  selectPlanOption(planOptionType,planOption)
{
	$.ajax({
        url:'/editplan/setplanoption',
        data:{plan_option_type:planOptionType,plan_option:planOption},
        dataType:"json"})
        .done(function(msg){
			response = eval(msg);
			if((planOptionType=="Theme")||(planOptionType=="Topic")) resetOtherOption(planOptionType);
			$('#plan'+planOptionType).html(response.code);
		   	$('.s-close-button').trigger("click");
			if(response.num_selections==1) {$('#goPlan').css("background-color","#f1c40f");$('#goPlanCaption').html("<div id='goPlanCaption'><p>Steady</p></div>");}
			if(response.num_selections>=2) {$('#goPlan').css("background-color","#2ecc71");$('#goPlanCaption').html("<div id='goPlanCaption'><a href=# onclick='makePlan()'>Go!</a></div>");}
         });
}
function resetOtherOption(planOptionType)
{
	if(planOptionType=="Theme") resetOptionType="Topic";
	else resetOptionType="Theme";
	resetHtml='<div id="plan'+resetOptionType+'"><p>Select<br>'+resetOptionType+'</p></div>';
	$('#plan'+resetOptionType).html(resetHtml);
}
function makePlan()
{
	$.ajax({
        url:'/editplan/makeplan',
        dataType:"json"})
        .done(function(msg){
			response = eval(msg);
			if(response.topic) topicTheme = "&plan_topic=" + response.topic;
			if(response.theme) topicTheme = "&plan_theme=" + response.theme;
			location.href = "http://" + location.host + "/editplan/planner2?plan_year=" + response.year + topicTheme;
         });
}
function adminProcess(proc)
{
    // This function sends the form data to the controller and reports response
    var url = "/admin/" + proc;
    var formName = "#admin" + proc + 'form';
    var divId = "#" + proc + "_div";
	var sendData=$(formName).serialize();
	$(".error").val('');
    $.post(url, {data:sendData},
        ...
        if(response.result==1){ 
				...
				classNum = response.class_num;
				if(response.school_approval==1) location.href = '/admin/sendschooluserinformation?school_id=' + schoolId;
				else if((!classNum)||(classNum==0)) location.href = response.referrer;
				else location.href = '/admin/createschoolusers?school_id=' + schoolId + '&class_num=' + classNum;
...
function adminCreateSchoolUsers(numClasses)
{
	var url = "/admin/createschoolusers";
	var sendData = new Array();
	if(numClasses > 1) {
		for(i=1;i<=numClasses;i++){
			var formName = '#admincreateschoolusersform_'+i;
			sendData = $(formName).serialize();
			$.post(url, {data:sendData},
        function(msg) {
				var response=eval(msg);
				// success
		...
		var formName = '#admincreateschoolusersform_1';
			sendData = $(formName).serialize();
			$.post(url, {data:sendData},
        function(msg) {
				var response=eval(msg);
...
{
	jConfirm('Are you sure you want to delete user with id = ' + userId + '?', 'Confirmation Dialog', function(r) {
    if (r) {
		$.ajax({
...
{
	searchFor=$('#user_search').val();
	/**$.ajax({
        url:'/admin/listusers',
		data:{search_for:searchFor},
        dataType:"json"})
        .done(function(msg){
			response = eval(msg);
			
         });**/
	location.href = "http://" + location.host + "/admin/listusers?search_for=" + searchFor;
}
function adminSearchSchool()
{
	searchFor=$('#school_search').val();
	/**$.ajax({
        url:'/admin/listusers',
		data:{search_for:searchFor},
        dataType:"json"})
	...
	location.href = "http://" + location.host + "/admin/listschools?search_for=" + searchFor;
}
function adminDeleteSchool(schoolId)
{
	jConfirm('Are you sure you want to delete school with id = ' + schoolId + '?', 'Confirmation Dialog', function(r) {
    if (r) {
		$.ajax({
...
function sendSchoolUserInformation()
{
	userIds = [];
	schoolId=$('#all_school').val();
	if($('#all_school').is(':checked'))
	{
		$.ajax({
        		url:'/admin/sendschooluserinformation',
				type:'POST',
				data:{school_id:schoolId},
        		dataType:"json"})
        			.done(function(msg){
						...
						if(response.result==1){ 
                			...
                			location.href = '/admin/editschool?school_id='+response.detail;
	...
	{
		$( "input[name='send_users[]']:checked" ).each(function( index ) {
			userIds[index] = $( this ).val();
		});
		if(userIds.length!=0) {
			dataToPost = JSON.stringify(userIds);
			$.ajax({
        		url:'/admin/sendschooluserinformation',
				type:'POST',
				data:{user_ids:dataToPost,school_id:schoolId},
        		dataType:"json"})
        			.done(function(msg){
						response = eval(msg);
						if(response.result==1){ 
