...
class Application_Model_DbTable_StoredProcedures extends Zend_Db_Table_Abstract 
{
	...
	public function CreateQuickPlan($teacher_id,$topic_id,$topic,$level,$description,$num_lessons,$plan_type)
	{
		if($plan_type=='setplan'){
        $pdo = $this->getAdapter()->prepare("CALL CreateQuickDefaultUnit(:teacher,:topic_id,:topic,:level,:description,:num_lessons,@success)");
                $pdo->bindParam(':teacher',$teacher_id,PDO::PARAM_STR);
                $pdo->bindParam(':topic_id',$topic_id,PDO::PARAM_STR);
                $pdo->bindParam(':topic',$topic,PDO::PARAM_STR);
                $pdo->bindParam(':level',$level,PDO::PARAM_STR);
	...
	public function CreateQuickGuestPlan($teacher_id,$topic_id,$topic,$level,$description,$num_lessons,$plan_type)
	{
		if($plan_type=='setplan'){
        $pdo = $this->getAdapter()->prepare("CALL CreateQuickDefaultGuestUnit(:teacher,:topic_id,:topic,:level,:description,:num_lessons,@success)");
                $pdo->bindParam(':teacher',$teacher_id,PDO::PARAM_STR);
                $pdo->bindParam(':topic_id',$topic_id,PDO::PARAM_STR);
                $pdo->bindParam(':topic',$topic,PDO::PARAM_STR);
                $pdo->bindParam(':level',$level,PDO::PARAM_STR);
