...
class CreateaplanController extends Zend_Controller_Action
{
    ...
    public function createPlanAction()
    {
        ...
        $plan_type=$_POST['plan_type'];
        if(($topic_selected=='55')&&(($_POST['sel_level']==2.5)||($_POST['sel_level']==3.5)))
		{
		$response['result']=1;
           $response['detail']='pdf';
           $response['more']=strtolower($topic_details_arr['name']);
		}
		elseif($topic_details_arr['status']=="P")
        {
           $response['result']=1;
           $response['detail']='pdf';
           $response['more']=strtolower($topic_details_arr['name']);
        ...
        elseif(!isset($mysession->create_limit))
        {
            if($plan_type=='setplan')
            {
                ...
                if($user_level==0) {
                    if ($create_plan->CreateQuickGuestPlan($teacher_id,$topic_id,'topic',$level,$title,$num_lessons,$plan_type)){
                    ...
                    if ($create_plan->CreateQuickPlan($teacher_id,$topic_id,'topic',$level,$title,$num_lessons,$plan_type)){
                    if(isset($mysession->curr_unit_id)) unset($mysession->curr_unit_id);
                    $response['result']=1;
                    $response['detail']='setplan';
                    ...
                    $response['detail']='problem with creation';
                       ...
                       $response['more']=$topic_details_arr;
                      }
            }
