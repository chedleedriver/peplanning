...
class StaffroomController extends Zend_Controller_Action
{
    ...
    public function init()
    {
			...
			$_GET['reason'] = (isset($_GET['reason']) ? $_GET['reason'] : 'default');
    }
    ...
    {        
		...
		if($lesson_array[0]['set_lesson_id']) $res_les_id = $lesson_array[0]['set_lesson_id']; else $res_les_id = $lesson_array[0]['id'];
        $resources = new Application_Model_DbTable_ContentResources();
        $lesson_resources=$resources->getLessonResources($res_les_id);
    ...
    public function showLessonDetailsAction()
    {
		...
		$lessons = new Application_Model_DbTable_Lesson();
		$set_lesson_id = $lessons->getSetLessonId($lesson_id);
		if($set_lesson_id) $res_les_id = $set_lesson_id->set_lesson_id; else $res_les_id = $lesson_id;
    ...
    public function editLessonAction()
    {
        ...
        elseif(isset($mysession->edit_limit)){
            ...
            $response['result']=1;
            $response['detail']="location.href=('/tplan/code/Lessons2014.php?p=".$urlParamsEnc."')";
